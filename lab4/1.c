...
int main() {
    ...
    if (pid == 0) {
        ...
        printf("%d",shmctl(shmid, IPC_RMID, NULL));
    }
...
/* 
1. Commented out `shmdt(shm_ptr)` in the child process:
...
2. Commented out `wait(NULL)` in the parent process:
...
3. Commented out `shmctl(shmid, IPC_RMID, NULL)` in the parent process:
...
5. Commented out the shared memory attachment `shmat(shmid, NULL, 0)` in either process:
...
6. Commented out `shm_ptr[i] = values[i]` in the child process:
